...
namespace SX_RFID.DataAcesss
{
    ...
    {
        #region【私用变量】
        private static string dbPath = ConfigurationManager.AppSettings["db_FilePath"];//要创建的SQLite数据库文件路径
        #endregion
        /// <summary>
        ...
        {//"Data Source=Test.db3;Pooling=true;FailIfMissing=false";
            get
            {
                ...
                string db_filepath = ConfigurationManager.AppSettings["db_FilePath"];
                if (string.IsNullOrEmpty(db_filepath))
                {
                    db_filepath = System.Windows.Forms.Application.StartupPath + "\\info.db";
                }
                var conectionstring = "Data Source=" + db_filepath + ";Pooling=true;FailIfMissing=false";
        ...
        /// <returns></returns>
        public static bool CheckIsExists(string tablename,string key,string value)
        {
            string checksql = "select " + key + " from " + tablename + " where " + key + " = '" + value + "'";
            object obj = ExecuteScalar(checksql, null);
            return (obj != null) ;
        }
